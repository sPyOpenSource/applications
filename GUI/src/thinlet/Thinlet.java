...
public class Thinlet extends Container
	...
	}
	{
		...
		if (MOUSE_WHEEL != 0) {
			...
			} catch (IllegalAccessException | IllegalArgumentException | NoSuchMethodException | SecurityException | InvocationTargetException exc) { /* never */ }
		}
		// set listeners flags
		enableEvents(AWTEvent.COMPONENT_EVENT_MASK |
			...
			try {
				if ((System.getProperty("java.vendor").contains("Insignia")) &&
					!System.getProperty("os.name").contains("Windows CE")) { evm = -1; }
		...
		if (null != classname) switch (classname) {
                ...
                case "tabbedpane":{
                    ...
                    for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                        ...
                        if (!stacked) {
                            if (horizontal) {
                                if ("bottom".equals(placement)) { r.y = bounds.height - tabsize; }
                                r.height = tabsize;
                            }
                            else {
                                if ("right".equals(placement)) { r.x = bounds.width - tabsize; }
                                r.width = tabsize;
                            }
                        }
                ...
                case "dialog":
                    ...
                    boolean border = ("panel".equals(classname)) && getBoolean(component, "border", false);
                        ...
                        for (int i = 0; comp != null; comp = get(comp, ":next")) {
                            ...
                            if ((!"fill".equals(halign)) || (!"fill".equals(valign))) {
                                ...
                                if (!"fill".equals(halign)) {
                                    ...
                                    if ("center".equals(halign)) { ix += dw / 2; }
                                    else if ("right".equals(halign)) { ix += dw; }
                                    iwidth -= dw;
                                }
                                if (!"fill".equals(valign)) {
                                    int dh = Math.max(0, iheight - d.height);
                                    if ("center".equals(valign)) { iy += dh / 2; }
                                    else if ("bottom".equals(valign)) { iy += dh; }
                                    iheight -= dh;
                                }
                            }
                ...
                case "desktop":{
                    ...
                    for (Object comp = get(component, ":comp");
                        ...
                        } else if ((!":combolist".equals(iclass)) && (!":popup".equals(iclass))) {
	...
	private Object popupMenu(Object component) {
		...
		if ((selected == null) || (!"menu".equals(getClass(selected)))) { return null; }
	...
	private void showTip() {
		...
		if (("tabbedpane".equals(classname)) || ("menubar".equals(classname)) || (":popup".equals(classname))) {
			if (insidepart != null) {
	...
	private Dimension getPreferredSize(Object component) {
		...
		if (("button".equals(classname)) || ("togglebutton".equals(classname))) {
			boolean link = ("button".equals(classname)) && (get(component, "type") == "link");
	...
	private void paint(Graphics g,
		...
		if (null == classname) throw new IllegalArgumentException(classname);
		else switch (classname) {
                case "label":
		...
		if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_MOVED) ||
			...
			if (port != null) { // is scrollable
				...
				try { // mouse wheel is supported since 1.4 thus it use reflection
					...
					int rotation = ((Integer) wheelrotation.invoke(e));
			...
			if (focusinside && ((popupowner != null) || (focusowner != null))) {
				...
				if ((control == (id == KeyEvent.KEY_PRESSED)) &&
					...
					for (Object splitpane = focusowner;
						...
						if ("splitpane".equals(getClass(splitpane))) {
	...
	private boolean processField(Object component,
		...
		if (editable && (keychar != 0) &&
			...
			} catch (HeadlessException | UnsupportedFlavorException | IOException exc) {
				insert = clipboard;
			}
	...
	private Object getPreviousFocusable(Object component,
		...
		if ((component != null) && (component != block)) {
			boolean go = ((!"dialog".equals(getClass(component))) ||
				(outgo && !getBoolean(component, "modal", false)));
			if (!start && !upward && go) {
	...
	private Object parse(InputStream inputstream,
		...
		try {
			...
			Vector<Object> methods = (validate && !dom) ? new Vector<>() : null;
	...
	private void addAttribute(Object component, String key, String value, Vector<Object> lasts) {
		...
		if ("string" == definition[0]) {
			...
			} catch (ClassNotFoundException | IllegalAccessException | InstantiationException exc) { throw new IllegalArgumentException(value); }
		}
		...
		if (value != null) {
			...
			try {
				...
				while (st.hasMoreTokens()) {
					...
					} catch (IllegalAccessException | IllegalArgumentException | NoSuchFieldException | SecurityException exc) { // not mask value
						keycode = KeyEvent.class.getField("VK_" + token).getInt(null);
					}
				}
				keystroke = ((long) modifiers) << 32 | keycode;
			} catch (IllegalAccessException | IllegalArgumentException | NoSuchFieldException | SecurityException exc) { throw new IllegalArgumentException(token); }
		}
		...
		for (int i = 0; i < n; i++) {
			...
			if ("thinlet".equals(arg)) {
				...
				if ("item".equals(compname)) {
					...
					if (null == parentclass) throw new IllegalArgumentException(parentclass + " has no item");
					else switch (parentclass) {
                                        case "list":
                                            classname = "item";
                                            break;
                                        case "tree":
