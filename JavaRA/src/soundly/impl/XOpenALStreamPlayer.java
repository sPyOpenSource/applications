 ...
 */
public class XOpenALStreamPlayer {
	...
	private ByteBuffer bufferData = ByteBuffer.allocate(sectionSize);
	/** The buffer holding the names of the OpenAL buffer thats been fully played back */
 	private IntBuffer unqueued = IntBuffer.allocate(1);
	 ...
	 */
	public XOpenALStreamPlayer(int source, String ref) {
		this.source = source;
		this.ref = ref;
		
		bufferNames = IntBuffer.allocate(BUFFER_COUNT);
	 ...
	 */
	public XOpenALStreamPlayer(int source, URL url) {
		this.source = source;
		this.url = url;
	...
	private void initStreams() throws IOException {
		...
		if (url != null) {
			//audio = new InputStream(url.openStream());
		} else {
			//audio = new InputStream(ResourceLoader.getResourceAsStream(ref));
		}
	...
	public void update() {
		...
		float sampleRate = 0;//audio.getRate();
		float sampleSize;
		...
		int processed = 0;//AL10.alGetSourcei(source, AL10.AL_BUFFERS_PROCESSED);
		while (processed > 0) {
			unqueued.clear();
			...
			float bufferLength = 0;//(AL10.alGetBufferi(bufferIndex, AL10.AL_SIZE) / sampleSize) / sampleRate;
	...
	public boolean stream(int bufferId) {
		try {
			...
			if (count != -1) {
				...
				int format = 0;//audio.getChannels() > 1 ? AL10.AL_FORMAT_STEREO16 : AL10.AL_FORMAT_MONO16;
				try {
	...
	public float getPosition() {
		return 0;//positionOffset + AL10.alGetSourcef(source, AL11.AL_SEC_OFFSET);
	}
}
