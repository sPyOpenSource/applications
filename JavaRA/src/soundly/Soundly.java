...
public class Soundly {
    ...
    private static DoubleBuffer sourcePos = DoubleBuffer.allocate(3);
    ...
    public static AudioData loadSound(String ref) throws Exception {
	SoundStore.get().init();
	Media audio = null;
	try {
		...
		throw new Exception("Use XStream for streaming .xm and .mod sounds");
	    } else {
	...
	return null;//new AudioData(audio.getBufferID());
    }

    public static AudioData loadSound(URL url) throws Exception {
	SoundStore.get().init();
    ...
    public static AudioData loadWavSound(String ref, InputStream in) throws Exception {
     ...
     * @return
     */
    public static double calculateDistance(ManagedSound sound) {
     ...
     * @return
     */
    public static double calculateDistance(double sX, double sY, double mX, double mY) {
     ...
     * @return
     */
    public static double calculateFromCenter(ManagedSound sound, double distance, float listenerRadius) {
     ...
     * @return
     */
    public static double calculateFromCenter(ManagedSound sound, double distance) {
    ...
    private DoubleBuffer sourceVel = DoubleBuffer.allocate(3).put(new double[] { 0.0f, 0.0f, 0.0f });
    /**
     * Position of the listener in 3D space.
     */
    private DoubleBuffer listenerPos = null;
	...
	listenerPos = DoubleBuffer.allocate(3).put( 
		new double[] { listenerX, listenerDepth, listenerY } );
	listenerPos.flip();
	//AL10.alListener(AL10.AL_POSITION, listenerPos);

	sourceVel.clear();
	sourceVel.put(new double[] { 0, 0, 0 });
	sourceVel.flip();
	...
	for (int i=startIndex; i<endIndex; i++) {
		...
		if (distanceCheckingEnabled && rad>0f) {
		    ...
		    if (!ignore) {
			double dist = Soundly.calculateDistance(oSound);
			double prox = Soundly.calculateFromCenter(oSound, dist, rad);
     ...
     * @param depth
     */
    public void setListenerPosition(double x, double y, float depth) {
	double oldX = this.listenerX;
	double oldY = this.listenerY;
	...
	if (oldX!=x||oldY!=y||oldZ!=depth) {
	    ...
	    //AL10.alListeneri( AL10.AL_POSITION, listenerPos);
	    distanceChanged();
	}
    }
	...
	if (source==-1)
	    return -1;
	return 0;//AL10.alGetSourcei(source, AL10.AL_BUFFER);
    }
	...
	if (source==-1)
	    return false;
	return false;//AL10.alGetSourcei(source, AL10.AL_LOOPING) == AL10.AL_TRUE;
    }
	...
	return false;//AL10.alGetSourcei(source, AL10.AL_SOURCE_RELATIVE) == AL10.AL_TRUE;
    }
     ...
     * @param gain  
     */
    public static void setSourceVolume(int source, double gain) {
	if (source==-1)
	    return;
	if (gain <= 0) 
	    gain = 0f;
	...
	if (source==-1)
	    return 1f;
	return 0;//AL10.alGetSourcef(source, AL10.AL_GAIN);
    } 
     ...
     * @param depth
     */
    public static void setSourcePosition(int source, double x, double y, double depth) {
	if (source==-1)
	    return;
	float s = getScale();
	sourcePos.clear();
	sourcePos.put(new double[] { x*s, depth*s, y*s });
	sourcePos.flip();
	...
	if (source==-1)
	    return -1;
	return 0;//AL10.alGetSourcef(source, AL11.AL_SEC_OFFSET);
    }
	...
	if (source==-1)
	    return false;
	return false;//AL10.alGetSourcei(source, AL10.AL_SOURCE_STATE) == AL10.AL_PLAYING;
    }
	...
	if (source==-1)
	    return false;
	return false;//AL10.alGetSourcei(source, AL10.AL_SOURCE_STATE) == AL10.AL_PAUSED;
    }
	...
	if (source==-1)
	    return false;
	return false;//AL10.alGetSourcei(source, AL10.AL_SOURCE_STATE) == AL10.AL_STOPPED;
    }
