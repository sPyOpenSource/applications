...
public class BuildingOverlay {
	...
	if (this.targetBuilding != null) {
		...
		for (Pos wallPos : this.currentWallsList) {
		    double x = wallPos.getX() * 24;
		    double y = wallPos.getY() * 24;
    ...
    public void drawCYCircles(Scene g) {
	...
	if (this.targetBuilding != null) {
	    boolean isBuildingCancelled = !GUI.getInstance().getPlayer().getBase().getProductionQueue().getProductionForBuilding(this.targetBuilding).isReady();

	    if (isBuildingCancelled || !GUI.getInstance().getPlayer().getBase().getProductionQueue().canBuild(this.targetBuilding)) {
		resetBuildingMode();
	    }
	}
    }
	...
	int cX = (int) (-GUI.getInstance().getCamera().getTranslateX() + newx) / 24;
	int cY = (int) (-GUI.getInstance().getCamera().getTranslateY() + newy) / 24;	
