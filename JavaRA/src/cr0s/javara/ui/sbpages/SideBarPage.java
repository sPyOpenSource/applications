...
public abstract class SideBarPage {
    private final ArrayList<SideBarItemsButton> buttons;
	...
	for (SideBarItemsButton button : this.buttons) {
	    ...
	    int barX = (int)GUI.getInstance().getContainer().getWidth() - mouseX - GameSideBar.BAR_SPACING_W;
		...
		// Draw tooltip
		EntityActor a = GUI.getInstance().getPlayer().getBase().getProductionQueue().getBuildableActor(button);
	...
	GUI.getInstance().getPlayer().getBase().productButtonItem(button);	

	GUI.getInstance().getSideBar().switchPage("start");
    }
}
