...
public class PerfomanceGraphRenderer {
    ...
    private static final Color BORDER_COLOR = Color.rgb(128, 128, 128, 128f/255);
    private static final Color BG_COLOR = Color.rgb(0, 0, 0, 200f/255);
    
    //static TrueTypeFont SMALL_FONT = new TrueTypeFont(new Font("Verdana", Font.PLAIN, 8), true);
    
    public static void render(Scene g, Pos pos) {	
	/*g.setColor(BG_COLOR);
	...
	int k = 0;
	double drawX = pos.getX();
	double drawY = pos.getY() + HEIGHT - 2;
	...
	for (Entry<String, ProfilingSection> item : Profiler.getInstance().profilers.entrySet()) {
		...
		double previousX = drawX + n * basis.getX();
		double previousY = drawY - previousValue * basis.getY();

		double currentX = drawX + (n + 1) * basis.getX();
		double currentY = drawY - currentValue * basis.getY();
