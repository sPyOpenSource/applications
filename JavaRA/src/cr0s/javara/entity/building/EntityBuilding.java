...
public abstract class EntityBuilding extends EntityActor {
    ...
    protected static BufferedImage repairImage;
    static {
	...
	if (this.isRepairing) {
		...
		this.repairingTicks = EntityBuilding.REPAIR_INTERVAL_TICKS;
		this.repairIconBlink = !this.repairIconBlink; // blink wrench icon

		if (this.getHp() + EntityBuilding.REPAIR_HP_AMOUNT >= this.getMaxHp()) {
		    if (this.owner.getBase().takeCash(EntityBuilding.REPAIR_COST)) {
		    ...
		    if (this.owner.getBase().takeCash(EntityBuilding.REPAIR_COST)) {
			this.setHp(this.getHp() + EntityBuilding.REPAIR_HP_AMOUNT);
		    } else {
			this.isRepairing = false;
			this.repairIconBlink = false;
			this.repairingTicks = 0;
