...
public class ScreenShaker {
	...
	if (!this.effects.isEmpty()) {
	    LinkedList<ShakeEffect> e = new LinkedList<>();
	...
	Pos cp = new Pos(
		-GUI.getInstance().getCamera().getTranslateX() + GUI.getInstance().getContainer().getWidth() / 2,
		-GUI.getInstance().getCamera().getTranslateY() + GUI.getInstance().getContainer().getHeight() / 2);
	...
	//GUI.getInstance().getCamera().scrollBy(offset);
    }
