...
public class Shroud {
    private final boolean explorationMap[][];
    
    private final World w;
    private final Player p;
    
    private final ShroudRenderer sr;
	...
	int maxX = 0;//Math.min(tileX + range, (int) (this.w.getMap().getBounds().getMaxX() / 24) + 1);
	int maxY = 0;//Math.min(tileY + range, (int) (this.w.getMap().getBounds().getMaxY() / 24) + 1);
	
	for (int x = Math.max((int) this.w.getMap().getBounds().getX() / 24, tileX - range); x < maxX; x++) {
	    for (int y = Math.max((int) this.w.getMap().getBounds().getY() / 24, tileY - range); y < maxY; y++) {
		int dx = tileX - x;
		int dy = tileY - y;
