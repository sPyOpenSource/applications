...
public class Theater {
    ...
    private HashMap<String, Pos> shpTexturesPoints = new HashMap<>();
	...
	try {
	    ib = new BufferedImage(SHEET_SIZE, SHEET_SIZE, BufferedImage.TYPE_INT_ARGB);
	} catch (Exception e1) {
	    e1.printStackTrace();
	}
    ...
    private void putTextureInSheet(BufferedImage sheet, ShpTexture texture) {
	if (texture == null) {
	    return;
	}
	...
	for (int sX = 0; sX + img.getWidth() < SHEET_SIZE; sX++) {
		...
		rect = new Rectangle(sX, sY, img.getWidth(), img.getHeight());

		// Image is fit in sheet on new position?
		if (rect.getX() + rect.getWidth() <= SHEET_SIZE - img.getWidth() && rect.getY() + rect.getHeight() <= SHEET_SIZE - img.getHeight()) {
		    // Check for intersection with other textures

		    boolean isIntersectsWithOthers = false;
		    for (Rectangle r : this.texturesBounds) {
			if (r.intersects(rect.getBoundsInLocal())) {
		    ...
		    try {
			sheet.getGraphics().drawImage(img, sX, sY, null);
		    } catch (Exception e) {
			e.printStackTrace();
		    }
		    ...
		    this.shpTexturesPoints.put(texture.getTextureName(), new Pos(sX, sY));

		    // Shift search positions
		    currentSheetX = (int) (rect.getX() + rect.getWidth());
		    currentSheetY = (int) rect.getY();
    ...
    private void putTextureInSheet(BufferedImage sheet, TmpTexture texture) {
	if (texture == null) {
	    return;
	}
	...
	texture.setSpriteSheetCoords(new Pos(currentSheetX, currentSheetY));

	for (int i = 0; i < texture.numImages; i++) {
	    BufferedImage img = texture.getByIndex(i);

	    int deployX = currentSheetX;
	    int deployY = currentSheetY + (i * (int)img.getHeight());

	    // Copy texture into sheet
	    try {
		sheet.getGraphics().drawImage(img, deployX, deployY, null);
	    } catch (Exception e) {
		e.printStackTrace();
	    }    
	}
    ...
    private void putShpTextureInSheetDissected(BufferedImage sheet, ShpTexture texture) {
	if (texture == null) {
	    return;
	}
	...
	this.shpTexturesPoints.put(texture.getTextureName(), new Pos(currentSheetX, currentSheetY));

	for (int i = 0; i < texture.numImages; i++) {
	    BufferedImage img = texture.getAsImage(i, null);
    ...
    public Pos getTileTextureSheetCoord(TileReference<Integer, Byte> tile) {
    ...
    public Pos getShpTexturePoint(String textureName) {
