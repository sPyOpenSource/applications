...
public class Camera {
	...
	public void init(Stage gc) throws Exception {
	...
	public void update(Stage gc, int delta) throws Exception {
		//setViewPort(-offsetX, -offsetY, gc.getWidth(), gc.getHeight());

		worldClip = new Rectangle(0, 0, gc.getWidth(), gc.getHeight());
	}

	public void render(Stage gc, Scene g) throws Exception {
	...
	public void renderFinish(Stage gc, Scene g) throws Exception {
	...
	public boolean isEntityInsideViewport(Entity e) {
		...
		return true;//viewportRect.intersects(e.boundingBox);
	}
