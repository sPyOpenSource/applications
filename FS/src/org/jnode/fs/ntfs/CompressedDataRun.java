...
public final class CompressedDataRun implements DataRunInterface {
    ...
    public int readClusters(long vcn, Memory dst, int dstOffset, int nrClusters, int clusterSize, NTFSVolume volume)
        ...
        if (copyDest + copyLength > dst.size()) {
            throw new ArrayIndexOutOfBoundsException(
                String
                    .format("Copy dest %d length %d is too big for destination %d", copyDest, copyLength, dst.size()));
        }
     ...
     */
    private static void unCompressUnit(final Memory compressed,
