...
class IdentifyOperation extends Operation {
    private static final byte CMD_IDENTIFY    = (byte)0xec;   // Identify drive
    private static final byte CMD_ATAPI_IDENT = (byte)0xa1;   // Querying CDROMs and tape drives
    ...
    public void startOperation() throws IDEException {
        ...
        if (((controller.getStatus() & (Controller.STATUS_RDY | Controller.STATUS_BSY)) == Controller.STATUS_RDY) 
            ...
            if (identifyCommand(CMD_ATAPI_IDENT) == -1) {
                Debug.out.println("" + drive.name + ": no answer (Status = " + controller.getStatus() + ")");
            }
            controller.getStatus(); // make sure that interrupt is prevented
        } else {
    ...
    */
    ...
    if (cmd == CMD_ATAPI_IDENT) {
        ...
        switch (type) {
            case 0x0:
            Debug.out.println("FLOPPY or CDROM - not supported");
            break;
            case 0x5:
            Debug.out.println("CDROM - not supported");
            break;
            case 0x1:
            Debug.out.println("TAPE - not supported");
            break;
            case 0x7:
            Debug.out.println("OPTICAL DISK - not supported");
            break;
            case 0x21:
            Debug.out.println("SCSI DISK - not supported");
            break;
            default:
            Debug.out.println("unknown device (Typ " + type + ")");
        }
        drive.present = false;
