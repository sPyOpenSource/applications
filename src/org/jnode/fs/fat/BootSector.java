...
public class BootSector {
    ...
    //private final PartitionEntry[] partitions;
    ...
    public BootSector(int size) {
        data = rm.alloc(size);
        dirty = false;
        //partitions = new PartitionEntry[4];
    }

    public BootSector(byte[] src) {
        data = rm.alloc(src.length);
     ...
     * @param device
     */
    /*public synchronized void read(BlockIO device) throws IOException {
        device.readSectors(0, data.size(), data, true);
     ...
     * @param device
     */
    /*public synchronized void write(BlockIO device) throws IOException {
        device.writeSectors(0, data.size(), data, true);
        dirty = false;
    }*/
    ...
    protected int get8(int offset) {
        return 0;//LittleEndian.getUInt8(data, offset);
    }
    ...
    protected int get16(int offset) {
        return data.getLittleEndian16(offset >> 1);
    }
    ...
    protected void set16(int offset, int value) {
        data.setLittleEndian16(offset >> 1, (short)value);;
        dirty = true;
    }
    ...
    protected long get32(int offset) {
        return data.getLittleEndian32(offset >> 2);
    }
    ...
    protected void set32(int offset, long value) {
        data.setLittleEndian32(offset >> 2, (int)value);;
        dirty = true;
    }
    ...
    /*public PartitionEntry initPartitions(Geometry geom, IBMPartitionTypes firstPartitionType)
        ...
        PartitionEntry entry = getPartition(0);
    ...
    public synchronized PartitionEntry getPartition(int partNr) {
        if (partitions[partNr] == null) {
            partitions[partNr] = new PartitionEntry(null, data, partNr);
        }
