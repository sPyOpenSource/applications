...
public class FatDirEntry extends FatBasicDirEntry implements Inode//, FSEntryCreated, FSEntryLastAccessed 
{
    ...
    public static FatBasicDirEntry fatDirEntryFactory(AbstractDirectory dir, Memory src, int offset) {
        int flags = 0;//LittleEndian.getUInt8(src, offset + 0x0b);
        ...
        if (r && h && s && v) {
            ...
            return null;// new FatLfnDirEntry(dir, src, offset);
        }
     ...
     * @param offset
     */
    public FatDirEntry(AbstractDirectory dir, Memory src, int offset) {
        super(dir, null, offset);

        this.parent = dir;
        id = Integer.toString(offset / FatConstants.DIR_ENTRY_SIZE);
        unused = (src.get8(offset) == 0);
        deleted = false;//(LittleEndian.getUInt8(src, offset) == 0xe5);

        char[] nameArr = new char[8];
        for (int i = 0; i < nameArr.length; i++) {
            nameArr[i] = 0;//(char) LittleEndian.getUInt8(src, offset + i);
        }
        ...
        for (int i = 0; i < extArr.length; i++) {
            extArr[i] = 0;//(char) LittleEndian.getUInt8(src, offset + 0x08 + i);
        }
        setExt(new String(extArr).trim());

        this.flags = 0;//LittleEndian.getUInt8(src, offset + 0x0b);
        this.created =
            DosUtils.decodeDateTime(src.getLittleEndian16((offset + 0x10) >> 1),
                src.getLittleEndian16((offset + 0x0e) >> 1));
        this.lastModified =
            DosUtils.decodeDateTime(src.getLittleEndian16((offset + 0x18) >> 1),
                src.getLittleEndian16((offset + 0x16) >> 1));
        this.lastAccessed =
            DosUtils.decodeDateTime(src.getLittleEndian16((offset + 0x12) >> 1), 0); // time not stored
        this.startCluster = src.getLittleEndian16((offset + 0x1a) >> 1);
        this.length = src.getLittleEndian16((offset + 0x1c) >> 1);
        this._dirty = false;
        //this.rights = new UnixFSAccessRights(getFileSystem());
    }
    ...
    public Directory getDirectory() throws IOException {
        if (isDirectory()) {
            return null;//getFatFile().getDirectory();
        } else {
