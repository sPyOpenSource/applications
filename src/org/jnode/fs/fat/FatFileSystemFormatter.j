...
public class FatFileSystemFormatter extends Formatter<FatFileSystem> {
    ...
    public FatFileSystem format(Device device) throws FileSystemException {
        try {
            ...
            BlockIO api = (BlockIO) device.getAPI(BlockIO.class);
            int sectorSize = api.getSectorSize();

            PartitionEntry entry = api.getPartitionTableEntry();

            // if we can deduce partitiontable/fat dependencies do it otherwise
            // guess it.
            if (entry != null && entry instanceof PartitionEntry) {
                numberOfSectors = entry.getNrSectors();
                offset = entry.getStartLba();
            } else {
                numberOfSectors = api.getCapacity() / sectorSize;
                offset = 0;
            }
