...
public class Fat {
     ...
     * @param sectorSize
     */
    public Fat(int mediumDescriptor, int nrSectors, int sectorSize) {
    ...
    public synchronized void read(BlockIO device, long offset) throws IOException {
        Memory data = rm.alloc(nrSectors * sectorSize);
        ...
        for (int i = 0; i < entries.length; i++) {
            ...
            /*switch (fatType) {
                ...
                case FAT32:
                    idx = i * 4;
                    long l1 = data.get8(idx);
                    long l2 = data.get8(idx + 1);
                    long l3 = data.get8(idx + 2);
                    long l4 = data.get8(idx + 3);
        ...
        for (int i = 0; i < entries.length; i++) {
            ...
            /*switch (fatType) {
                case FAT12: 
                    idx = (int) (i * 1.5);
                    if ((i % 2) == 0) {
                        data.set8(idx, (byte) (v & 0xFF));
                        data.set8(idx + 1, (byte) ((v >> 8) & 0x0F));
                    } else {
                        data.set8(idx, (byte)(data.get8(idx) | ((v & 0x0F) << 4)));
                        data.set8(idx + 1, (byte) ((v >> 4) & 0xFF));
                    }
                    break;
                case FAT16: 
                    idx = i << 1;
                    data.set8(idx, (byte) (v & 0xFF));
                    data.set8(idx + 1, (byte) ((v >> 8) & 0xFF));
                    break;
                case FAT32:
                    idx = i << 2;
                    data.set8(idx, (byte) (v & 0xFF));
                    data.set8(idx + 1, (byte) ((v >> 8) & 0xFF));
                    data.set8(idx + 2, (byte) ((v >> 16) & 0xFF));
                    data.set8(idx + 3, (byte) ((v >> 24) & 0xFF));
                    break;
            }*/
    ...
    protected boolean isReservedCluster(long entry) {
        return false;//fatType.isReservedCluster(entry);
    }
    ...
    protected boolean isEofCluster(long entry) {
        return false;//fatType.isEofCluster(entry);
    }
