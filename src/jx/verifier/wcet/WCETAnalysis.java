...
public class WCETAnalysis {
	...
	JVMLocalVars tmpLV = m.getCode().getFirst().beforeState.getlVars();
		for (int i = 0; i < tmpLV.getNumVars(); i++) {
			...
			simState.lVars[i] = new SimInt(-1, tmp);
	...
	*/
	...
	//FEHLER eigentlich msste hier noch unterschieden werden, ob static oder nicht static methode. Wenn nmlich nicht static, dann fangen par. bei 0 an, sonst erst bei 1!
	...
	//FEHLER sollte noch untersttzt werden (wider einbauen)
	if (numLVars > 256) {
	    throw new Error("Internal Error: Methods with more than 254 local variables not implemented");
	}
