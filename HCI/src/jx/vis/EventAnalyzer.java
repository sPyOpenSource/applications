...
public class EventAnalyzer {
	...
	Debug.out.println("-graph <filename>: Create file <filename> that contains the times between events as an event graph and can be used as input for the dot program.");
        Debug.out.println("-count <filename>: Create file <filename> that contains the number of events and can be used as input for the gnuplot program.");
        Debug.out.println("-countfreq <filename> <interval>: Create file <filename> that contains the number of events sampled over <interval> and can be used as input for the gnuplot program.");
        Debug.out.println("-maxevents <number>: Only process number events.");
        Debug.out.println("-timescale <number>: 1=nano, 2=micro, 3=milli, 4=sec");
        Debug.out.println("-threadid: info1 contains thread id");
    }
	...
	if (opt_graph) {
		...
		out.println(d.from + "->" + d.to + " [label=\""+((int)(d.median*timefactor))+" +/- "+((int)(d.deviation*timefactor))+" ("+d.number+")\"];");
            ...
            for (EventType type : e.types) {
                ...
                if (found) {
                    out.println(type.number + " [label=\"" + type.name + "\"];");
                }
            }
	    out.println("}");
	...
	if (starteventname!=null){
            for (EventType type : e.types) {
                if (type.name.equals(starteventname)) {
                    starteventnumber = type.number;
                    break;
                }
            }
	}	
