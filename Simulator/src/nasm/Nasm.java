...
public class Nasm{
	...
	for (Map.Entry< String, TsItemVar> me:st){
		...
		ajoutePseudoInst(new Resd(new Label(identif), tsItem.type.taille(), "variable globale"));
	    if(tsItem instanceof TsItemVarTab)
		ajoutePseudoInst(new Resd(new Label(identif), tsItem.type.taille() * ((TsItemVarTab)tsItem).taille, "variable globale"));
