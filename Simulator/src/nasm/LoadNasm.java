...
public class LoadNasm extends DepthFirstAdapter {
    ...
    @Override
    public void caseAOrgConst(AOrgConst node){
        inAOrgConst(node);
        node.getNumconst().apply(this);
        //pseudoInst = new Org(nb,"");
        outAOrgConst(node);
    }
    @Override
    public void caseAEquConst(AEquConst node){
        inAEquConst(node);
        node.getLabel().apply(this);
        
        node.getNumconst().apply(this);
        //pseudoInst = new Equ(label,nb,"");
        outAEquConst(node);
    }
    ...
    {
        inALabel(node);
        label = new Label(node.getId().getText());
        outALabel(node);
    }
}
