  ...
  public int read32(int address) {
    if (byteOrder)
      return ((int)mem[address]&0xFF)+(((int)mem[address+1]&0xFF)<<8)
        +(((int)mem[address+2]&0xFF)<<16)+(((int)mem[address+3]&0xFF)<<24);
    return (((int)mem[address]&0xFF)<<24)+(((int)mem[address+1]&0xFF)<<16)
    +(((int)mem[address+2])<<8)+((int)mem[address+3]);
  }
