...
public class SSAEmul extends Emulator {
  ...
  private boolean insLoadVal(int reg0, int reg1, int size) {
    ...
    // because of return address and additional push&pop in insEnter/-Leave
    // the parameters are further away than specified in rela
    if (reg1==SSADef.R_BASE) addr+=(curInlineLevel>0 ? paramOffsetInline : paramOffsetNormal);
    switch (size) {
      case 1:
  ...
  private boolean insConv(int reg0, int reg1, int size, int para) {
    ...
    switch (r0.size) {
      ...
      case 4:
        switch (r1.size) {
          case 1: 
            r0.value32=(int)r1.value8;
            break;
          case 2:
            r0.value32=(int)r1.value16;
            break;
          case 3:
            r0.value32=(int)((char)r1.value16);
            break;
          case 4:
      ...
      case 5:
        switch (r1.size) {
          case 1: 
            r0.value32=real.buildFloat((long)r1.value8, 0);
            break;
          case 2:
            r0.value32=real.buildFloat((long)r1.value16, 0);
            break;
          case 3:
            r0.value32=real.buildFloat((long)((char)r1.value16), 0);
            break;
          case 4:
            r0.value32=real.buildFloat((long)r1.value32, 0);
            break;
          case 5:
            r0.value32=r1.value32;
            break;
      ...
      case 8:
        switch (r1.size) {
          case 1: 
            r0.value64=(long)r1.value8;
            break;
          case 2:
            r0.value64=(long)r1.value16;
            break;
          case 3:
            r0.value64=(long)((char)r1.value16);
            break;
          case 4:
            r0.value64=(long)r1.value32;
            break;
          case 5:
      ...
      case 9:
        switch (r1.size) {
          case 1: 
            r0.value64=real.buildDouble((long)r1.value8, 0);
            break;
          case 2:
            r0.value64=real.buildDouble((long)r1.value16, 0);
            break;
          case 3:
            r0.value64=real.buildDouble((long)((char)r1.value16), 0);
            break;
          case 4:
            r0.value64=real.buildDouble((long)r1.value32, 0);
            break;
          case 5:
  ...
  private boolean insPushImI(int size, int imI) {
    ...
    else stck.ptr=(stck.ptr-size) & ~stackClearBits;
    switch (size) {
      case 1:
  ...
  private boolean insAssign(int reg0, int reg1, int size) {
    ...
    switch (size) {
      case 1: //byte and boolean
        ...
        break;
      case 2: case 3: //short and char
        write16(false, r0.ptr, r1.value16);
        break;
      case 4: case 5: //int and float
        write32(false, r0.ptr, r1.value32);
        break;
      case 8: case 9: //long and double
  ...
  private boolean insNot(int reg0, int reg1, int size) {
    ...
    switch (size) {
      case 1:
        r0.value8=(byte)((int)~r1.value8);
        break;
      case 2: case 3:
        r0.value16=(short)((int)~r1.value16);
        break;
      case 4:
        r0.value32=~r1.value32;
        break;
  ...
  private boolean insJump(int cond, int rela) {
    ...
    if (jump) {
      if (killRegOnJmp1!=0) {
        if (!insKillReg(killRegOnJmp1)) return false;
        killRegOnJmp1=0;
      }
      if (killRegOnJmp2!=0) {
        if (!insKillReg(killRegOnJmp2)) return false;
        killRegOnJmp2=0;
      }
      if (curRegStartOff==0)
        lastAllocRegIndex=0;
  ...
  private boolean insAllocReg(int reg0, int size, boolean restore) {
    ...
    if (curRegStartOff==0) curRegStartOff=reg0;
      ...
      cnt=regs.length;
      while (reg0-curRegStartOff>=cnt) cnt*=2;
    ...
    regs[cnt].size=size;
    if (cnt>highestUsedRegEntry) highestUsedRegEntry=cnt;
  ...
  private boolean insKillReg(int reg0) {
    ...
    r.size=0;
    if (reg0==killRegOnJmp1) killRegOnJmp1=0;
    if (reg0==killRegOnJmp2) killRegOnJmp2=0;
...
//    if (reg0>0) for (r=reg0; r<=reg1; r++) getRegister(r, 0, false).size=0;
  ...
  private boolean insThrowFrameUpdate(boolean secondCall, int tfo, int destRela) {
    ...
    if (secondCall) write32(false, sReg[SSADef.R_BASE].ptr+tfo+relocBytes,
        currentIP+destRela);
    return true;
  }
  ...
  private boolean insThrowFrameReset(int reg0, int tbo) {
    ...
    write32(false, r0.ptr,
        globalThrowFrameVariable=read32(false, sReg[SSADef.R_BASE].ptr+tbo));
    return true;
  }
  ...
  private boolean insDoThrow() {
    ...
    write32(false, globalThrowFrameVariable+relocBytes*6,
          ...
          switch (opcode) {
            ...
            case 0xFF&SSADef.I_CALLind: 
              pars="r".concat(toDecString(reg0)).concat(",").
                concat(toDecString(iPar1)).concat(",parSize==").concat(toDecString(iPar2));
            ...
            case 0xFF&SSADef.I_CMP:
              pars=getType(iPar1).concat("r").concat(toDecString(reg0)).
                concat(",r").concat(toDecString(reg1)).concat(",par==").concat(toDecString(iPar2));
            ...
            case 0xFF&SSADef.I_IN:
              ...
              break;
            case 0xFF&SSADef.I_BOUND:
              pars="r".concat(toDecString(reg1)).concat(",[r").
                concat(toDecString(reg0)).concat("+").concat(
                    toHexString(iPar1)).concat("]");
              mn=new Mnemonic(tempIP, "bound    ", pars, isJumpDest);
              break;
            case 0xFF&SSADef.I_DEREF:
              pars="r".concat(toDecString(reg0)).concat(",[r").concat(
                  toDecString(reg1)).concat("+r").concat(
                      toDecString(reg2)).concat("*(").concat(
                          toDecString(iPar2)).concat(")+").concat(
                              toHexString(iPar1)).concat("]");
            ...
            case 0xFF&SSADef.I_EXCEPT:
              ...
              break;
            case 0xFF&SSADef.I_IVOF:
              pars=toDecString((int)lPar).concat(",").concat(toDecString((int)(lPar>>>32))).concat(
                  ",").concat(toDecString(iPar1)).concat(",").concat(toDecString(iPar2));
  ...
  public int getStartOfMethod(int currIP) {
    ...
    do {
      ...
      if (opcode==(0xFF&SSADef.I_INLINE)) { //MAGIC.inline
        ...
        if ((param&SSADef.IP_reg1)!=0) currentIP+=4;
        if ((param&SSADef.IP_reg2)!=0) currentIP+=4;
        if ((param&SSADef.IP_size)!=0) currentIP++;
        else if ((param&SSADef.IP_im_i)!=0) iPar1=readInt();
        if ((param&SSADef.IP_para)!=0) currentIP+=4;
        if ((param&SSADef.IP_im_l)!=0) currentIP+=8;
        // process instruction
        if (opcode==(0xFF&SSADef.I_LEAVE)) leaveFound=true;
        else if (leaveFound) {
          goOn=false;
          if (opcode==(0xFF&SSADef.I_MARKER)) //marker found
            res=currentIP-iPar1;
          else res=-1; //MARKER not found after LEAVE, signal error
        }
      }
    } while (goOn);
    currentIP=saveIP;
  ...
  public int getEndOfMethod(int currIP) {
    ...
    while (true) {
      ...
      if (opcode == (0xFF & SSADef.I_INLINE)) { //MAGIC.inline
        ...
        if ((param&SSADef.IP_reg2)!=0) currentIP+=4;
        if ((param&SSADef.IP_size)!=0) currentIP++;
        else if ((param&SSADef.IP_im_i)!=0) currentIP+=4;
        if ((param&SSADef.IP_para)!=0) currentIP+=4;
        if ((param&SSADef.IP_im_l)!=0) currentIP+=8;
        // process instruction
