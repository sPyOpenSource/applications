...
public class BoxWorld extends JComponent implements WindowListener, KeyListener,
    ...
    // current world
    private final int[][] state = new int[X_SIZE][Y_SIZE];
...
//            menu.show(this, e.getX(), e.getY());
    ...
    private List<Point> findPath(Point p1, Point p2) {
        ...
        HashMap<Point, Point> vmap = new HashMap<>();
        nlist.add(p1);
...
//        beep.setState(beepOn);
    ...
    private static final String[] help = {"s - start game", "n - next world",
    ...
    private void undoController(int key) {
        if (isUndo) {
            redoStack.push(invert(key));
        } else {
            if (redoStack.size() > 0 && !isRedo)
                redoStack.removeAllElements();
            undoStack.push(invert(key));
        }
    }
